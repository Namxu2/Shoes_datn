...
@Component
public class SoleServicelmpl  implements SoleService{
    ...
    public Sole createSole(CreateSoleRequest createSoleRequest){
        ...
        if (sole != null){
            throw new BadRequestException("Tên đế giày đã tồn tại trong hệ thống, vui lòng kiểm tra lại!");
...
//        } catch (Exception ex) {
//            throw new InternalServerException("Lỗi khi chỉnh sửa đế giày");
//        }
//    }
@Override
public void updateSole(CreateSoleRequest createSoleRequest, Long id) {
    ...
    Sole sl = soleRepository.findByName(createSoleRequest.getName());
    // Kiểm tra xem sl có khác null không trước khi gọi equals
    if (sl != null) {
        Long existingId = sl.getId(); // Lấy ID của đế giày hiện tại
        Long requestId = createSoleRequest.getId(); // Lấy ID từ request

        // Kiểm tra null trước khi gọi equals
        if (requestId != null && existingId != null && !requestId.equals(existingId)) {
            throw new BadRequestException("Tên đế giày " + createSoleRequest.getName() + " đã tồn tại trong hệ thống, Vui lòng chọn tên khác!");
        }
    }
